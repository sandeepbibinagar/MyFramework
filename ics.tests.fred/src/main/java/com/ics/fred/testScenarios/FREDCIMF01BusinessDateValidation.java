...
public class FREDCIMF01BusinessDateValidation extends ICSDBUtilis {
	
	public static void validAndInvalidCIBusinessDateValidation(String serverName,String dbName,String sqlConfigBusinessDate,
			...
			String templateFileType,String templateCreditFileName,
		...
		createFileFromTemplate(templateFilePath, templateCreditFileName,".sql", "STRINGTOBEREPLACEDININPUTFILE", extractID);		
		createFileFromTemplate(templateFilePath, templateCreditFileName+"_Actual",".sql", "BUSINESSDATETOBEREPLACE", actualConfigDateValue);
		
		createFileFromTemplate(templateFilePath, "BuisnessDateValidation05MF01",".sql", "STRINGTOBEREPLACEDININPUTFILE", extractIDWithouHypen);			
		createFileFromTemplate(templateFilePath, "BuisnessDateValidation05MF01_Actual",".sql", "BUSINESSDATETOBEREPLACE", dummyVal);
	   ...
	   // String nextDayDate = GenericMethods.ConvertDateInToRequiredFormat("yyyy-MM-dd","yyyyMMdd",date.toString());
		...
		createFileFromTemplate(templateFilePath, "BuisnessDateValidationFormatIncorrect05MF01",".sql", "STRINGTOBEREPLACEDININPUTFILE", extractIDWithouHypen);		
		createFileFromTemplate(templateFilePath, "BuisnessDateValidationFormatIncorrect05MF01_Actual",".sql", "BUSINESSDATETOBEREPLACE", dateIncorrectFormatVal);
		
		 String sqlBusinessDateFileMultipleInjectCommand = "cmd /c sqlcmd -S "+serverName+" -d "+dbName+" -i "+
				 templateFilePath + templateCreditFileName + "_Actual.sql "+
	...
	//	 validationStepInformation("BusinessDate Fetched from Database is "+actualConfigDateValue+" and updated businessdate in .SQL File");
		...
		//***** TestStep 2: To Verify correct BusinessDate is ingested in database for the file having correct business date as per TestStep1****//
    	 ...
    	 String sqlBusinessDateFileDebitInjectCommand = "cmd /c sqlcmd -S "+serverName+" -d "+dbName+" -i "+templateFilePath + templateCreditFileName + "_Actual.sql ";
 		// sqlCommandExecution(sqlBusinessDateFileDebitInjectCommand);
 		 
 		 validationStepInformation(templateCreditFileName+"_Actual.sql has been processed into database.");
...
for(int i=1;i<=bdateArray.size();i++){
	...
	case 2:
		...
		String mf01SendQueueCastQuery ="SELECT TOP 3 CAST(message_body AS NVARCHAR(MAX)) FROM "+dbName+".[Base].[//FRED/MF01SendQueue] where validation='X' order by message_enqueue_time desc";
	...
	case 3:
		...
		String queryVal ="SELECT TOP 3 CAST(message_body AS NVARCHAR(MAX)) FROM ["+dbName+"].[Base].[//FRED/MF01SendQueue] where validation='X' order by message_enqueue_time desc";
